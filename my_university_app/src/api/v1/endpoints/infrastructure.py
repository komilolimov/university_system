...
# ИСПРАВЛЕНО: Добавлено право buildings:write
@building_router.post("/", response_model=BuildingRead, dependencies=[Depends(RequirePermission(["buildings:write"]))])
def create_building(obj_in: BuildingCreate, session: SessionDep):
    return building_service.create(session=session, obj_in=obj_in)

# ИСПРАВЛЕНО: Добавлено право buildings:write
@building_router.put("/{building_id}", response_model=BuildingRead, dependencies=[Depends(RequirePermission(["buildings:write"]))])
...
# ИСПРАВЛЕНО: Добавлено право buildings:delete
@building_router.delete("/{building_id}", dependencies=[Depends(RequirePermission(["buildings:delete"]))])
...
# ИСПРАВЛЕНО: Добавлено право rooms:write
@room_router.post("/", response_model=RoomRead, dependencies=[Depends(RequirePermission(["rooms:write"]))])
def create_room(obj_in: RoomCreate, session: SessionDep):
    return room_service.create(session=session, obj_in=obj_in)

# ИСПРАВЛЕНО: Добавлено право rooms:write
@room_router.put("/{room_id}", response_model=RoomRead, dependencies=[Depends(RequirePermission(["rooms:write"]))])
...
# ИСПРАВЛЕНО: Добавлено право rooms:delete
@room_router.delete("/{room_id}", dependencies=[Depends(RequirePermission(["rooms:delete"]))])
