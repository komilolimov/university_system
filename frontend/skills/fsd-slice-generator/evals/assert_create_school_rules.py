...
def main():
    if len(sys.argv) < 2:
        print("Usage: python assert_create_school_rules.py <output_dir>")
        sys.exit(1)
    ...
    for f in ts_files:
        with open(f, 'r', encoding='utf-8') as file:
            ...
            if 'apiClient.POST' in content or 'apiClient.post' in content or ('apiClient' in content and 'POST' in content):
                has_api_client_post = True
                evidence.append(f"Found apiClient POST usage in {f}")
    ...
    grading = {
        "expectations": [
            ...
            {
                "text": "Uses apiClient for POST request",
                "passed": has_api_client_post,
